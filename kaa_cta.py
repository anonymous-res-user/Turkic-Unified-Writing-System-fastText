...
t1 = Transliterator("kaa", Script.LATIN, Script.COMMON_TURKIC)
t2 = Transliterator("kaa", Script.CYRILLIC, Script.COMMON_TURKIC)
...
file_path = "rawwikitxt/kaa_20260101.txt"
print(file_path)
linecount = 0

pure_uroman_list = []
common_turkic_list = []
# CHV_COMPAT_MAP = str.maketrans({
#     unicodedata.normalize("NFC", "ӑ"): unicodedata.normalize("NFC", "ă"),
#     unicodedata.normalize("NFC", "Ӑ"): unicodedata.normalize("NFC", "Ă"),
#     unicodedata.normalize("NFC", "ӗ"): unicodedata.normalize("NFC", "ĕ"),
#     unicodedata.normalize("NFC", "Ӗ"): unicodedata.normalize("NFC", "Ĕ"),
# })
...
with open(file_path, "r", encoding="utf-8") as rawwikitxt_f:
    for each_rawwikitxt_line in rawwikitxt_f:
        ...
        # each_rawwikitxt_line = each_rawwikitxt_line.translate(CHV_COMPAT_MAP)
        # each_rawwikitxt_line = unicodedata.normalize("NFC", each_rawwikitxt_line)
...
with open("uroman_kaa.txt", 'w', encoding='utf-8') as uromanf:
    for each_pureroman in pure_uroman_list:
        uromanf.write(' '.join(tokenize(each_pureroman))+'\n')

with open("comturk_kaa.txt", 'w', encoding='utf-8') as comturkf:
